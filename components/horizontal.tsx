      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        ...
        <motion.div style={{ x }} className="flex gap-4 relative">
          {cards.map((card, index) => (
            <Card card={card} key={card.id} priority={index === 0} />
          ))}
...
const Card = ({
  ...
  return (
    <div className="group relative h-[450px] w-[450px] overflow-hidden bg-neutral-200 rounded-lg">
      <Image
        src={card.url}
        alt={card.title}
        fill
        ...
        blurDataURL="/placeholder.png" // 🔧 Optional tiny base64 blur image
        priority={priority}
      />
    </div>
  );
